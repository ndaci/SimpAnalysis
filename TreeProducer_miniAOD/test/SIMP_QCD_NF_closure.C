...
void SIMP_QCD_NF_closure(){
  ...
  double jet_pt[8], jet_eta[8], jet_phi[8], jet_efrac_ne_Had[8], jet_efrac_ne_EM[8], NEF_jet[8];
	...
	TFile* efficiencies = new TFile("eff2D_QCD_nPixHits_photonVeto_NHEF_PUMoriond17.root", "READ");
	TH2D* eff_histos[11];
	for(int j = 0; j < 11; j++){
  ...
  TFile *output = new TFile("closure_test_logratio_nPixHitsCut_photonVeto_NHEF_PUMoriond17_miniAOD.root", "RECREATE");
...
// 				if (track_ptError[0]/track_pt[0] < 0.5){
					for(int j = 0; j < 11; j++){
						if (NEF_jet[0]>nf_cuts[j] && NEF_jet[1]>nf_cuts[j]){
	...
	// 					std::cout<<"eff2 = "<<eff2<<std::endl;
						if (NEF_jet[1]>nf_cuts[j]) passed_eff1[j]+=weight*eff1;
						if (NEF_jet[0]>nf_cuts[j]) passed_eff2[j]+=weight*eff2;
...
// 				} 
  ...
  pad1->Draw();
  pad1->cd();
	TGraphErrors *MC = new TGraphErrors(11, nf_cuts, passed_MCtruth, zero, err_MCtruth);
	...
	TGraphErrors *oneleg = new TGraphErrors(11, nf_cuts, passed_eff, zero, err_eff);
...
// 	oneleg2->Draw("P");
	...
	TGraphErrors *twoleg = new TGraphErrors(11, nf_cuts, passed_effboth, zero, err_effboth);
  ...
  pad2->cd();
	...
	TGraphErrors *ratio_oneleg = new TGraphErrors(11, nf_cuts, ratio_1leg, zero, err_ratio_1leg);
  ...
  ratio_oneleg->GetYaxis()->SetTitle("#frac{1/2-leg}{MCtruth}");
	...
	TGraphErrors *ratio_twoleg = new TGraphErrors(11, nf_cuts, ratio_2leg, zero, err_ratio_2leg);
	ratio_twoleg->SetMarkerStyle(20);
	ratio_twoleg->SetMarkerColor(3);
	ratio_twoleg->Draw("P");
  
  TLine *line = new TLine(0.45,1,1.05,1);
