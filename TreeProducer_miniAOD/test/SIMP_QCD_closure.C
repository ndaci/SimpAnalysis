...
void SIMP_QCD_closure(){
  ...
  double jet_pt[4], jet_eta[4], jet_phi[4], jet_efrac_ch_Had[4], jet_efrac_ch_EM[4], jet_efrac_ch_Mu[4];
	...
	TFile* efficiencies = new TFile("eff2D_QCD_random_0p1.root", "READ");
	TH2D* eff_histos[11];
	for(int j = 0; j < 11; j++){
  ...
  TFile *output = new TFile("closure_errorbars_correct_random_0p1.root", "RECREATE");
	...
	//   chain->SetBranchAddress("jet_m", &M);
		...
		for(Int_t entry = 0; entry < Nentries; ++entry){
			...
			if (jet_pt[0] > 250 && jet_pt[1] > 250 && fabs(jet_eta[0]) < 2.0 && fabs(jet_eta[1]) < 2.0 && deltajet_phi > 2){
				for(int j = 0; j < 11; j++){
					if (CHEF_jet[0]<chf_cuts[j] && CHEF_jet[1]<chf_cuts[j]) passed_MCtruth[j]+= weight;
					double eff1 = eff_histos[j]->GetBinContent(eff_histos[j]->GetXaxis()->FindBin(jet_eta[0]), eff_histos[j]->GetYaxis()->FindBin(jet_pt[0]));
					double eff2 = eff_histos[j]->GetBinContent(eff_histos[j]->GetXaxis()->FindBin(jet_eta[1]), eff_histos[j]->GetYaxis()->FindBin(jet_pt[1]));
// 					std::cout<<chf_cuts[j]<<std::endl;
// 					std::cout<<eff_histos[j]<<std::endl;
...
// 					std::cout<<"eff2 = "<<eff2<<std::endl;
					...
					if (CHEF_jet[0]<chf_cuts[j] && CHEF_jet[1]<chf_cuts[j]) err_MCtruth[j] += pow(weight, 2);
// 					err_eff1[j] += ;
// 					err_eff2[j] += ;
// 					err_effboth[j] += ;
				}
