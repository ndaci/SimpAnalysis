...
void SIMP_QCD_closure_peretabin(double eta_min, double eta_max, TString outputname){
	...
	for (int l = 0; l < 6; l++){
		...
		for(Int_t entry = 0; entry < Nentries; ++entry){
			...
			if (jet_pt[0] > 250 && jet_pt[1] > 250 && fabs(jet_eta[0]) < 2.0 && fabs(jet_eta[1]) < 2.0 && deltajet_phi > 2){
				if(jet_eta[0] > eta_min && jet_eta[1] > eta_min && jet_eta[0] < eta_max && jet_eta[1] < eta_max){
					for(int j = 0; j < 11; j++){
