...
void SIMP_Data_macro(){
  ...
  TFile *output = new TFile("plots_Data_AOD.root", "RECREATE");
...
// 		Int_t entries = 3000000;
  ...
  for(Int_t entry = 0; entry < entries; ++entry){
    ...
    if (corrjets.HLT_DiCentralPFJet170 == 1 && corrjets.jet_pt[0] > 250 && corrjets.jet_pt[1] > 250 && fabs(corrjets.jet_eta[0]) < 2.0 && fabs(corrjets.jet_eta[1]) < 2.0 && deltajet_phi > 2 /*&& corrjets.track_nPixHits[0] > 0*/ && (corrjets.photon_passLooseId[0] == 0 || (corrjets.photon_passLooseId[0] == 1 && dR1 > 0.1 && dR2 > 0.1))){
      ...
      if (CHEF_corrjet[0] > 0.25 && CHEF_corrjet[1] > 0.25){
        if (dRjet1 < 0.4){
...
//       ndofvsCHF2->Fill(CHEF_corrjet[1], corrjets.track_ndof[0], weight);
      ...
      if(corrjets.photon_pt[0] > 10 && corrjets.photon_passLooseId[0] == 1) photon1_pt->Fill(corrjets.photon_pt[0], corrjets.pswgt_dijet_170);
      if(corrjets.photon_pt[0] > 10 && corrjets.photon_passLooseId[0] == 1) photon1_eta->Fill(corrjets.photon_eta[0], corrjets.pswgt_dijet_170);
      if(corrjets.photon_pt[0] > 10 && corrjets.photon_passLooseId[0] == 1) photon1_phi->Fill(corrjets.photon_phi[0], corrjets.pswgt_dijet_170);
    }    
	}
  output->Write();
  output->Close();
