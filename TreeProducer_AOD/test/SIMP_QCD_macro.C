...
void SIMP_QCD_macro(){
  ...
  TFile *output = new TFile("QCD_plots_prescaledlumi0p062_PUMoriond17_dzCut1_AOD.root", "RECREATE");
...
// 	double QCD_events[5] = {16830696, 19199088, 15621634, 4980387, 3846616};
	...
	for (int j = 0; j < 5; j++){
		...
		for(Int_t entry = 0; entry < Nentries; ++entry){
			...
			if (jet_pt[0] > 250 && jet_pt[1] > 250 && fabs(jet_eta[0]) < 2.0 && fabs(jet_eta[1]) < 2.0 && deltajet_phi > 2 && track_dzError[0]/track_dz[0] < 1){
