...
// 	TRandom3 r;
  ...
  double genjet_pt[4], genjet_eta[4], genjet_phi[4], genjet_efrac_ch[4];
  ...
  TFile *output = new TFile("eff2D_QCD_GenJets_PUMoriond17_AOD.root", "RECREATE");
...
// 			Double_t random1 = r.Uniform();
			...
			double deltajet_phi = genjet_phi[0] - genjet_phi[1];
...
// 			if (jet_pt[0] > 250.0 && jet_pt[1] > 250.0 && fabs(jet_eta[0]) < 2.0 && fabs(jet_eta[1]) < 2.0 && deltajet_phi > 2.0 && track_dzError[0]/track_dz[0] < 10){
			if (genjet_pt[0] > 250.0 && genjet_pt[1] > 250.0 && fabs(genjet_eta[0]) < 2.0 && fabs(genjet_eta[1]) < 2.0 && deltajet_phi > 2.0){
// 				if (track_ptError[0]/track_pt[0] < 0.5){
// 					if(CHEF_jet[0] > 0.5){
					if(genjet_efrac_ch[0] > 0.5){
						total->Fill(fabs(genjet_eta[1]), genjet_pt[1], weight);
						for(int j = 0; j < 11; j++){
// 							if (CHEF_jet[1]<chf_cuts[j]) passed[j]->Fill(fabs(jet_eta[1]), jet_pt[1], weight);
							if (genjet_efrac_ch[1]<chf_cuts[j]) passed[j]->Fill(fabs(genjet_eta[1]), genjet_pt[1], weight);
						}
					}
// 					if(CHEF_jet[1] > 0.5){
					if(genjet_efrac_ch[1] > 0.5){
						total->Fill(fabs(genjet_eta[0]), genjet_pt[0], weight);
						for(int j = 0; j < 11; j++){
// 							if (CHEF_jet[0]<chf_cuts[j]) passed[j]->Fill(fabs(jet_eta[0]), jet_pt[0], weight);
							if (genjet_efrac_ch[0]<chf_cuts[j]) passed[j]->Fill(fabs(genjet_eta[1]), genjet_pt[1], weight);
						}
					}
