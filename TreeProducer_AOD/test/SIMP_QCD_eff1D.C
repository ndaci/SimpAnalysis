...
// 	list_QCD_2000ToInf(chain5);
  ...
  double jet_pt[4], jet_eta[8], jet_phi[8], jet_efrac_ch_Had[8], jet_efrac_ch_EM[8], jet_efrac_ch_Mu[8];
	double CHEF_jet[8], track_dzError[10], track_dz[10];
// 	E[4], M[4];
...
// 	double QCD_events[5] = {16830696, 19199088, 15621634, 4980387, 3846616};
  ...
  TFile *output = new TFile("eff1D_QCD_dzCut_PUMoriond17_AOD.root", "RECREATE");
...
// 			Double_t random1 = r.Uniform();
			...
			if (jet_pt[0] > 250 && jet_pt[1] > 250 && fabs(jet_eta[0]) < 2.0 && fabs(jet_eta[1]) < 2.0 && deltajet_phi > 2.0 && track_dzError[0]/track_dz[0] < 10){
